...
结构规则会查真实家目录、Keil 用户文件后缀、SID、私钥、常见凭据、完整 git hash 和真实邮箱；
运行时还会把本机账号名、设备名加入私人词，并扫描 Git refs 与提交元数据。分支名、雇主、代号
这类无法从形状判断的词，仍从 `<private-audit-wordlist>` 读取，故意不放进仓库，否则闸门
自己就是泄漏点。该文件不存在时，私人词表这一半跳过并说明原因，高置信结构规则照常执行。
